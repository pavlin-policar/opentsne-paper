...
%DIF DEL ../2022-11-08-initial-submission-2/paper/main.tex   Tue Nov  8 11:40:52 2022
%DIF ADD main.tex                                            Fri May 26 18:56:28 2023
...
In \proglang{Python}, the most commonly used implementation of the t-SNE algorithm comes from \pkg{scikit-learn}~\citep{pedregosa2011scikit}, which is nicely integrated with \proglang{Python} data science environment but suffers from poor scalability. Other implementations from the \proglang{R}~\citep{team2013r,krijthe2015rtsne} and \proglang{Julia}~\citep{bezanson2017julia,julia_tsne} programming languages also scale poorly (see Sec.~\ref{s:speed}). \pkg{MulticoreTSNE}~\citep{Ulyanov2016} enjoys better scalability but is likewise limited when applied to larger data sets. Additionally, none of these implementations include recently proposed extensions that can result in more globally consistent embeddings. \pkg{FIt-SNE}~\citep{linderman2019fast} alleviates these problems and provides bindings to both the \proglang{Python} and \proglang{R} programming languages. However, \DIFdelbegin %DIFDELCMD < \pkg{FIt-SNE} %%%
\DIFdel{is difficult to install and distribute and provides an API largely inconsistent with the }%DIFDELCMD < \proglang{Python} %%%
\DIFdel{data science ecosystem. None }\DIFdelend \DIFaddbegin \DIFadd{none }\DIFaddend of the existing t-SNE implementations support adding new samples into existing embeddings. To alleviate all these problems, and to support newly proposed additions to t-SNE algorithms, we here present \opentsne, an open-source \proglang{Python} implementation of the t-SNE algorithm. \opentsne is easy to install and includes precompiled binaries available through popular \proglang{Python} package managers. It provides a familiar API, making it suitable as a drop-in replacement for existing t-SNE implementations.
...
\DIFaddbegin \DIFadd{Alternatively, this gradient can be rewritten as
}\begin{equation}
\DIFadd{\frac{\partial C}{\partial \mathbf{y}_i} = 4 \Bigg [
...
\DIFdelbegin \DIFdel{Evaluation of }\DIFdelend \DIFaddbegin \DIFadd{A direct evaluation of the }\DIFaddend t-SNE gradients requires $\mathcal{O}(N^2)$ operations, which makes it impractical with large data sets and calls for efficient approximation schemes. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{van2014accelerating} }\hskip0pt%DIFAUXCMD
observed that }\DIFdelend \DIFaddbegin \DIFadd{The formulation in Equation~\ref{eq:grad_attr_rep} casts }\DIFaddend the t-SNE gradient \DIFdelbegin \DIFdel{can be cast }\DIFdelend as an N-body \DIFdelbegin \DIFdel{problem }\DIFdelend \DIFaddbegin \DIFadd{simulation }\DIFaddend where data points \DIFdelbegin \DIFdel{represent particles that attract and repel each
other. The gradient from Equation~\ref{eq:tsne_gradient} can be rewritten as }\begin{displaymath}
\DIFdel{\frac{\partial C}{\partial \mathbf{y}_i} = 4 \Bigg [
\underbrace{\sum_{j \neq i} p_{ij} q_{ij} Z \left ( \mathbf{y}_i - \mathbf{y}_j \right )}_{\text{attractive forces}}  -
\underbrace{\sum_{j \neq i} q_{ij}^2 Z \left ( \mathbf{y}_i - \mathbf{y}_j \right )}_{\text{repulsive forces}}
\Bigg ], \label{eq:grad_attr_rep} \notag
}\end{displaymath}%DIFAUXCMD
\DIFdel{where $Z = \sum_{k \neq l}\left ( 1 + || \mathbf{y}_k - \mathbf{y}_l ||^2 \right
)^{-1}$. We can view this equation as a particle simulation, where the
two terms represent the }\DIFdelend \DIFaddbegin \DIFadd{act as particles exerting }\DIFaddend attractive and repulsive forces \DIFdelbegin \DIFdel{between individual
particles. Each term lends itself }\DIFdelend \DIFaddbegin \DIFadd{onto one another. Both terms lend themselves }\DIFaddend to efficient approximations, enabling us to reduce the time complexity of \DIFaddbegin \DIFadd{the }\DIFaddend t-SNE \DIFaddbegin \DIFadd{algorithm }\DIFaddend to $\mathcal{O}(N)$. This allows modern t-SNE implementations to be leveraged for the visualization of data sets containing up to millions of data points.
...
initialization schemes based on PCA or \DIFdelbegin \DIFdel{spectral embeddings}\DIFdelend \DIFaddbegin \DIFadd{Laplacian eigenmaps~\mbox{%DIFAUXCMD
\citep{belkin2001laplacian}}\hskip0pt%DIFAUXCMD
}\DIFaddend , the clusters identified in the resulting embedding are typically grouped in a globally
coherent manner.
...
\begin{enumerate}
\item The t-SNE algorithm requires \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend specification of an affinity model to describe the similarities between data points \DIFaddbegin \DIFadd{$p_{ij}$}\DIFaddend . Standard t-SNE \DIFdelbegin \DIFdel{predominantly deals with tabular data and computes similarities using }\DIFdelend \DIFaddbegin \DIFadd{uses perplexity-based Gaussian kernels with varying bandwidths as specified in }\DIFaddend Equation~\ref{eq:p_ij}. However, \DIFaddbegin \DIFadd{other affinity models can also be used to highlight different aspects of the data or visualize other data modalities. For instance, }\DIFaddend \citet{kobak2019art} showed that using mixtures of Gaussians \DIFdelbegin \DIFdel{instead of a single Gaussian often reveals better global consistency of }\DIFdelend \DIFaddbegin \DIFadd{often better reveals global cluster organization in }\DIFaddend the resulting visualization (albeit at some computational cost). Alternatively, \DIFdelbegin \DIFdel{one can speed up the overall computation by choosing a uniform kernel, which yields quantitatively similar results }\DIFdelend \DIFaddbegin \DIFadd{a uniform affinity model can produce quantitively similar embeddings }\DIFaddend to standard t-SNE \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{at a third of the computational cost, making it particularly useful for larger data sets. Unlike the standard affinity model, which uses Gaussian kernels to determine the similarities $p_{ij}$, the uniform affinity model uses a uniform kernel to assign equal similarities to a pre-specified number of nearest neighbors. Visualizing different data modalities, such as graphs, can also be achieved by providing precomputed distance matrices that might consist of, for instance, the shortest paths between nodes. The }\DIFaddend \code{openTSNE.affinity} \DIFdelbegin \DIFdel{provides several affinity models to choose from }\DIFdelend \DIFaddbegin \DIFadd{module provides a selection of commonly used affinity models }\DIFaddend but defaults to the standard perplexity-based \DIFdelbegin \DIFdel{model }\DIFdelend \DIFaddbegin \DIFadd{Gaussian model, as }\DIFaddend proposed in the original publication. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In }%DIFDELCMD < \opentsne%%%
\DIFdel{, it is straightforward to define and use a custom affinity model. For instance, one can extend }\DIFdelend \DIFaddbegin \DIFadd{Users can also define custom affinity models by extending }\DIFaddend the \code{openTSNE.affinity.Affinities} class\DIFdelbegin \DIFdel{and provide an affinity model for relational data using a normalized adjacency matrix. Such extension enables rapid experimentation and opens the doors to the visualization of non-tabular data}\DIFdelend . \DIFaddbegin \DIFadd{The resulting class can then be used as a drop-in replacement for the built-in affinity models.
}\DIFaddend 

\item The optimization in t-SNE starts with initial positions for each data point in the embedding space. We can set the initial positions randomly or impose an initial global structure on the embedding\DIFdelbegin \DIFdel{with, for example, a PCA-based initialization. Accordingly, the }\DIFdelend \DIFaddbegin \DIFadd{. %DIF >  with, for example, a PCA-based initialization.
The }\DIFaddend \code{openTSNE.initialization} module provides several possible initialization schemes \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{including random, PCA-based, and Laplacian Eigenmap-based intialization schemes. }\DIFaddend The user can also specify their \DIFaddbegin \DIFadd{own }\DIFaddend initialization by providing a \code{numpy.ndarray} object. By default, \opentsne opts for PCA-based initialization.

\item In optimizing the embedding, the t-SNE algorithm finds the positions of the points in the embedding space that reflect their corresponding similarities in the affinity model. \opentsne stores the initial positions and affinities in \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend \code{openTSNE.TSNEEmbedding} object that includes the \code{.optimize} method for embedding optimization that uses either the Barnes-Hut or FIt-SNE approximation scheme. While the FIt-SNE approximation scheme scales linearly with the number of samples, it introduces additional computational overhead, often unnecessary for smaller data sets, resulting in longer \DIFaddbegin \DIFadd{overall }\DIFaddend runtimes than the \DIFdelbegin \DIFdel{Barnes-hut }\DIFdelend \DIFaddbegin \DIFadd{Barnes-Hut }\DIFaddend approximation scheme. By default, \opentsne uses \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an empirically-determined }\DIFaddend heuristic to choose the fastest algorithm for a given data set \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{based on its number of samples. If the data set contains fewer than 10,000 samples, the Barnes-Hut approximation scheme is selected, otherwise, the FIt-SNE is used.
}
...
\end{CodeInput}
\end{CodeChunk}
\opentsne provides precompiled binaries for all major platforms (Windows, macOS, Linux) for all currently supported \proglang{Python} versions (3.7---3\DIFdelbegin \DIFdel{.10}\DIFdelend \DIFaddbegin \DIFadd{.11}\DIFaddend ). Precompiled binaries are especially convenient for Windows users, as they obviate the need for a \proglang{C}/\proglang{C++} compiler, which is typically not bundled with Windows operating system.
...
\DIFdelbegin \DIFdel{0.6.2
}\DIFdelend \DIFaddbegin \DIFadd{1.0.0
}\DIFaddend \end{CodeInput}
\end{CodeChunk}
...
\begin{CodeChunk}
\begin{CodeInput}
>>> embedding.optimize(250, exaggeration=12, \DIFdelbegin \DIFdel{momentum=0.5, }\DIFdelend inplace=True)
>>> embedding.optimize(500, \DIFdelbegin \DIFdel{momentum=0.8, }\DIFdelend inplace=True)
>>> embedding[:3]
TSNEEmbedding([[-0.55581917, 17.56162344],
               [-1.76742536, 20.07763453],
               [-0.58444608, 20.00052495]])
...
\DIFdelbegin \DIFdel{momentum=0.8,
}\DIFdelend ...     max_grad_norm=0.25, inplace=True
... )
>>> new_embedding[::3]
PartialTSNEEmbedding([[-0.02637324, 15.69956674],
                      [-2.84200792, 16.15478451],
...
\end{CodeInput}
\end{CodeChunk}
\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{We here use the }\code{embedding.prepare_partial} \DIFadd{method to determine initial positions for the new data points, then call the }\code{new_embedding.optimize} \DIFadd{method to run the optimization process. Again, we can }\DIFaddend validate that this also produces the exact same embedding as before:
\begin{CodeChunk}
\begin{CodeInput}
...
\begin{figure*}[htbp]
  \center
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.75\textwidth]{tasic2018}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{tasic2018}
  \DIFaddendFL \caption{\label{fig:tasic}
...
embedding_ee = embedding.optimize(n_iter=500, exaggeration=12\DIFdelbegin \DIFdel{, momentum=0.5}\DIFdelend )
embedding_c = embedding_ee.optimize(n_iter=500, exaggeration=4\DIFdelbegin \DIFdel{, momentum=0.8}\DIFdelend )
embedding_b = embedding_c.optimize(n_iter=500, exaggeration=2\DIFdelbegin \DIFdel{, momentum=0.8}\DIFdelend )
embedding_a = embedding_b.optimize(n_iter=500, exaggeration=1\DIFdelbegin \DIFdel{, momentum=0.8}\DIFdelend )
\end{CodeInput}
\end{CodeChunk}
...
\begin{figure*}[ht]
  ...
  We select the fastest available, open-source implementations of the t-SNE algorithm from the \proglang{Python}, \proglang{R}, and \proglang{Julia} programming languages. We choose \opentsne (\DIFdelbeginFL \DIFdelFL{v0.6.2}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{v1.0.0}\DIFaddendFL ) for \proglang{Python}, \pkg{Rtsne}~\citep{krijthe2015rtsne} (v0.15) for \proglang{R}, and \pkg{TSne.jl}~\citep{julia_tsne} (v1.3.0) for \proglang{Julia}. The benchmarks reflect the time complexity of the implemented approximation schemes. For instance, \pkg{TSne.jl} implements a naive $\mathcal{O}(N^2)$ algorithm, making it prohibitively expensive for all but the smallest of data sets. \pkg{Rtsne} scales similarly to \pkg{MulticoreTSNE} in \proglang{Python}, as it implements the $\mathcal{O}(N \log N)$ Barnes-Hut approximation scheme. Interestingly, the multi-threaded implementation version of \pkg{Rtsne} crashes on benchmark data sets containing over 500,000 data points due to a lack of stack memory. We found these crashes to be inconsistent and somewhat random. \opentsne is the only library implementing the more recent FIt-SNE approximation scheme, which makes it suitable for larger data sets. }
\end{figure*}
...
\DIFaddbegin \subsubsection{Comparison to UMAP}

\DIFadd{In this section, we compare the performance of the }\opentsne \DIFadd{(v1.0.0) and }\pkg{umap-learn} \DIFadd{(0.5.3) libraries. }\pkg{umap-learn} \DIFadd{is the official }\textsf{\DIFadd{Python}} \DIFadd{implementation of the UMAP algorithm~\mbox{%DIFAUXCMD
\citep{2018arXivUMAP}}\hskip0pt%DIFAUXCMD
, another dimensionality reduction technique used for the visualization of high-dimensional data. A throughout comparison of the two algorithms is outside the scope of this manuscript, and we here instead highlight only their similarities and differences as they relate to the runtime of the implementations.
}

\DIFadd{Both t-SNE and UMAP are instances of force-directed layout algorithms in which each data point acts as a particle, attracting and repelling other particles. In the first step, both algorithms use the high-dimensional data set to construct a $k$-nearest neighbors graph (KNNG). Then, the low-dimensional embedding is optimized such that data points connected in the KNNG attract one another, and all data points exert some small, repulsive force. The two methods differ in the KNNG construction process, the specification of the attractive and repulsive forces, and their optimization approaches.
}

\DIFadd{The primary factor dictating the runtime of both methods is the number of nearest neighbors used when constructing the KNNG. Apart from the runtime required to find the $k$-nearest neighbors, the number of neighbors directly influences the runtime of each optimization step. In each step, every data point exerts an attractive force on each of its neighbors in the low-dimensional embedding. Therefore, the larger the number of neighbors considered, the longer the runtime. Modern implementations of the t-SNE algorithm construct a KNNG with $3 \cdot u$ nearest neighbors where $u$ is the desired perplexity. By default, }\opentsne \DIFadd{(v1.0.0) uses a perplexity value of 30, meaning that 90 nearest neighbors need to be found and considered for each data point. In contrast, the }\pkg{umap-learn} \DIFadd{package (v0.5.3) constructs a KNNG using only 15 neighbors by default, greatly reducing the time spent on nearest neighbor search and point interactions compared to t-SNE.
}

\DIFadd{Alternatively, empirical evidence indicates that replacing the standard perplexity-based Gaussian affinity model in t-SNE with a uniform affinity model produces markedly similar embeddings to that of standard t-SNE. For instance, running standard t-SNE with perplexity $u=30$ constructs a KNNG with $3 \cdot u = 90$ neighbors. However, when using the equivalent uniform affinity model, only $u$ neighbors are needed. This makes the uniform affinity model an attractive option for the visualization of large data sets.
}

\DIFadd{In Fig.~\ref{fig:benchmarks_umap}, we include benchmarks of the standard perplexity-based t-SNE algorithm, its uniform affinity model variant, and UMAP. Differently from the previous benchmarks which were run using older parameter settings to accomodate different t-SNE implementations, we now run both libraries using their default parameter settings to measure the runtime of typical, real-world use-cases.
}\begin{figure*}[ht]
  \centering
  \includegraphics[width=0.75\textwidth]{benchmarks_umap}
  \caption{\label{fig:benchmarks_umap} \DIFaddFL{We benchmark }\opentsne \DIFaddFL{(v1.0.0) against }\pkg{umap-learn} \DIFaddFL{(v0.5.3), the official }\textsf{\DIFaddFL{Python}} \DIFaddFL{implementation of the UMAP algorithm. Notice that the standard single-threaded perplexity-based variant of t-SNE runs slower than UMAP but outperforms UMAP in the multi-threaded case. We also benchmark the uniform affinity model variant of t-SNE, which outperforms both standard t-SNE and UMAP.}}
\end{figure*}

\DIFadd{Given that, by default, UMAP considers six times fewer neighbors than standard t-SNE, it is unsurprising that the single-threaded variant of UMAP outperforms the single-threaded implementation of standard t-SNE. Suprisingly, however, the single-threaded uniform affinity variant of t-SNE outperforms not only than the single-threaded variant of UMAP, but also its multi-threaded variant, despite dealing with half as many neighbors. This result likely stems from the fact that UMAP is written in pure }\textsf{\DIFadd{Python}}\DIFadd{, but uses }\pkg{numba}\DIFadd{~\mbox{%DIFAUXCMD
\citep{lam2015numba} }\hskip0pt%DIFAUXCMD
to accelerate computation. On the other hand, critical elements of the }\opentsne \DIFadd{library are written in }\pkg{cython}\DIFadd{~\mbox{%DIFAUXCMD
\citep{behnel2011cython}}\hskip0pt%DIFAUXCMD
, which is subsequently transpiled into }\textsf{\DIFadd{C++}}\DIFadd{, which may be able to be more aggresively optimized by the }\textsf{\DIFadd{C++}} \DIFadd{compiler. Another contributing factor may be the efficiency of the approximation schemes. While both the UMAP and FIt-SNE algorithms are asymptotically linear in the number of samples, their real-world runtimes may differ significantly.
}

\DIFadd{The multi-threaded benchmarks show that }\opentsne \DIFadd{outperforms UMAP in both its standard and uniform variants. The reason for this likely lies in the different optimization procedures used by the two algorithms. At each step of the optimization, }\opentsne \DIFadd{computes all pairwise interactions between data points in the embedding. While a direct calculation of these interactions would result in quadratic time complexity, }\opentsne \DIFadd{implements efficient approximation schemes that reduce this to linear time complexity (see Sec.~\ref{sec:meth.approx}). Importantly, these approximation schemes are very amenable to parallelization. This is clearly demonstrated by the runtime differences between the single-core and multi-core performance of }\opentsne\DIFadd{. On the other hand, UMAP bypasses the need to compute all pairwise interactions between data points using negative sampling. This allows UMAP to scale linearly in the number of samples without the need for additional approximation schemes. However, negative sampling is less amenable to paralellization and does not benefit from the availability of additional cores as much as t-SNE.
}

\DIFadd{Lastly, we turn our attention to an often overlooked runtime metric -- the runtime per optimization step. Both t-SNE and UMAP comprise two steps. First, both methods construct a KNNG, which runtime is dominated by $k$-nearest neighbor search. In the second step, a low-dimensional embedding is found via optimization. By default, }\opentsne \DIFadd{runs optimization for 750 iterations. UMAP, on the other hand, uses a heuristic based on the data set size to determine the number of iterations. For data sets containing fewer than 10,000 data points, UMAP opts for 500 iterations. For larger data sets, however, this is reduced to 200 iterations. Fig.~\ref{fig:benchmarks_umap} indicates that even in the single-threaded scenario, the perplexity-based variant of }\opentsne \DIFadd{requires significantly less time per iteration compared to }\pkg{umap-learn}\DIFadd{, as each }\opentsne \DIFadd{benchmark runs $3.75$ times more iterations than }\pkg{umap-learn} \DIFadd{despite considering more neighbors at each optimization step.
}

\DIFaddend \subsection{Ease of use}
...
\DIFdelbeginFL \DIFdelFL{Native }%DIFDELCMD < \proglang{Python} %%%
\DIFdelFL{installation }%DIFDELCMD < & \checkmark & & & \checkmark & & \\
%DIFDELCMD < %%%
\DIFdelendFL \pkg{PyPI} package & \checkmark & \checkmark & \DIFaddbeginFL \checkmark \DIFaddendFL & \checkmark & & \\
\pkg{conda} package & \checkmark & & \DIFaddbeginFL \checkmark \DIFaddendFL & \checkmark & & \\
...
%DIF < Multiscale Gaussian kernels & & & & \checkmark & \checkmark & & \\
...
\caption{\label{tab:features}
  We compare the features of \opentsne~(\DIFdelbeginFL \DIFdelFL{v0.6.2}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{v1.0.0}\DIFaddendFL ) to \pkg{scikit-learn}~(v1.0.1),
...
\pkg{openTSNE}~\DIFdelbegin \DIFdel{v0.6.2 }\DIFdelend \DIFaddbegin \DIFadd{v1.0.0 }\DIFaddend package. All results and figures can be reproduced using
...
\DIFadd{We here describe the preprocessing steps applied to the single-cell RNA-seq data sets used in Fig.~\ref{fig:macosko}~\mbox{%DIFAUXCMD
\citep{macosko2015highly}}\hskip0pt%DIFAUXCMD
, Fig.~\ref{fig:tasic}~\mbox{%DIFAUXCMD
\citep{tasic2018shared}}\hskip0pt%DIFAUXCMD
, Fig.~\ref{fig:cao}~\mbox{%DIFAUXCMD
\citep{cao2019single}}\hskip0pt%DIFAUXCMD
, and Fig.~\ref{fig:transform}~\mbox{%DIFAUXCMD
\citep{hochgerner2018conserved}}\hskip0pt%DIFAUXCMD
. The standard single-cell preprocessing pipeline consists of gene filtering, gene selection, data normalization, and dimensionality reduction.
}

\DIFadd{First, we discard genes detected fewer than ten times, then select the 3,000 most highly-variable genes following the gene-selection procedure from \mbox{%DIFAUXCMD
\citet{kobak2019art}}\hskip0pt%DIFAUXCMD
. We then perform counts-per-median library-size normalization followed by log normalization and standardization without zero-centering. Finally, we extract the top 50 principal components, which are then used to construct subsequent t-SNE embeddings. This pipeline is applied to all the aforementioned data sets with the following exceptions, which serve to better illustrate our points:
}\begin{itemize}
  \item \DIFadd{In \mbox{%DIFAUXCMD
\citet{macosko2015highly}}\hskip0pt%DIFAUXCMD
, we perform library-size normalization by computing counts-per-million and apply centering during standardization.
  }\item \DIFadd{In \mbox{%DIFAUXCMD
\citet{hochgerner2018conserved}}\hskip0pt%DIFAUXCMD
, we select the 1,000 most variable genes.
}\end{itemize}

\DIFadd{When embedding additional data points into the existing embedding in Figs.~\ref{fig:macosko}.c and \ref{fig:transform}, we follow the procedure from \mbox{%DIFAUXCMD
\citet{policar2021embedding}}\hskip0pt%DIFAUXCMD
. Therefore, no preprocessing is applied to the data sets from \mbox{%DIFAUXCMD
\citet{shekhar2016comprehensive} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{harris2018classes}}\hskip0pt%DIFAUXCMD
, and the similarities between the reference data set and the new data set are computed on the raw counts.
}
