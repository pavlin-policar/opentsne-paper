...
However, easy access to efficient implementations almost universally correlates with the widespread adoption of novel computational techniques. Despite the many theoretical advancements, most popular t-SNE libraries have been slow to incorporate them into their implementations. In this paper, we present \opentsne, our open-source \proglang{Python} implementation of t-SNE, complete with its recently proposed extensions. \opentsne\ is easy to install and includes precompiled binaries available through popular \proglang{Python} package managers. It provides a familiar API, making it suitable as a drop-in replacement for existing t-SNE implementations.
...
We introduce \opentsne, a comprehensive \proglang{Python} library that implements the t-SNE algorithm and its numerous recently proposed extensions. \opentsne\ is designed to accommodate all types of users and includes both a simple API that allows users to quickly obtain informative t-SNE visualizations, as well as a more flexible API that allows seasoned practitioners to modify all the
...
\begin{figure*}[htbp]
  ...
  The high-level structure and flow of the \opentsne\ library. Solid black lines show the standard parameter flow, while red, dashed lines indicate optional parameters, which allow for a finer level of control over the t-SNE algorithm.
  }
\end{figure*}

Fig.~\ref{fig:diagram} shows the high-level structure and flow of the \opentsne\ library.

Through different parameters, the user is given full control over every aspect of the t-SNE visualization algorithm, from different ways to define affinities to the delicate tuning of optimization parameters. This allows for rapid experimentation and enables users to quickly and efficiently inspect their data from numerous angles and at varying levels of resolution.

While \opentsne\ allows for an unprecedented level of control at every step, most end-users often only need a quick and easy way to create informative visualizations of their data sets. \opentsne\ provides a high-level API that closely follows the style of \pkg{scikit-learn}~\citep{sklearn_api}, a popular and widely used toolkit in the machine learning community. This allows users of all levels to quickly pick up and make use of the \opentsne\ library.
...
\opentsne\ is designed to be easy to use and can be installed through either \pkg{PyPI} using \pkg{pip}
\begin{CodeChunk}
\begin{CodeInput}
...
\end{CodeInput}
\end{CodeChunk}
\opentsne\ provides precompiled binaries for all major platforms (Windows, MacOS, Linux) for all currently supported \proglang{Python} versions (3.7---3.10). This is especially convenient for Windows users, as this obviates the need for a \proglang{C}/\proglang{C++} compiler, which is typically not bundled with the operating system.
...
We demonstrate the simplest usage of \opentsne\ on the Iris data set from \citet{anderson1936species}. This small data set describes 150 iris flowers containing four variables (sepal length, sepal width, petal length, petal width) and their species (Iris Setosa, Iris Versicolor, Iris Virginica), which is the target variable. This data set is included in \pkg{scikit-learn}:
\begin{CodeChunk}
\begin{CodeInput}
...
\end{CodeInput}
\end{CodeChunk}
Notice that the embedding object contains the same point coordinates as the initialization. This is because although we have created an \code{openTSNE.TSNEEmbedding} object, we have not yet run any optimization. To optimize the embedding, we merely have to call the \code{embedding.optimize} method using our desired parameter settings. Here, we use the default parameter settings, including a shorter early-exaggeration phase, followed by a longer phase with no exaggeration.
\begin{CodeChunk}
\begin{CodeInput}
...
lies on a lower-dimensional manifold, which can accurately be captured by a small number of dimensions. However, there is no evidence that every data set can
...
single-cell gene expression in mouse brain~\citep{tasic2018shared}. We generate
the embeddings using the following code snippet (shortened for clarity):
\begin{CodeChunk}
\begin{CodeInput}
...
Figs.~\ref{fig:cao}.a-c were generated using the following code snippet, which demonstrates how we can take advantage of the more advanced features of \opentsne\ to more quickly create meaningful visualizations of massive data sets. We first take a sample of the data and use t-SNE to construct a globally consistent initialization, which will then be used to initialize the remaning data points:
\begin{CodeChunk}
\begin{CodeInput}
...
\end{CodeInput}
\end{CodeChunk}
We then generate the three different embeddings using the following code snippet
...
code necessary to generate Fig.~\ref{fig:macosko}:
\begin{CodeChunk}
\begin{CodeInput}
...
Fig.~\ref{fig:transform}, we construct a reference embedding (using the same code snippet as above) using labeled data
...
% We benchmark Python
Fig.~\ref{fig:benchmarks_py} benchmarks four popular \proglang{Python}~\citep{vanrossum1995python} t-SNE
...
% Problem with Python
In \proglang{Python}, the most widely-used implementation of t-SNE comes from \pkg{scikit-learn}, which exhibits long runtimes when compared to other \proglang{Python} implementations. \pkg{scikit-learn}, like its \proglang{C++} counterpart -- \pkg{MulticoreTSNE}~\citep{Ulyanov2016} , is based on the Barnes-Hut approximation scheme. However, as shown on Fig.~\ref{fig:benchmarks_py}, \pkg{scikit-learn} exhibits much longer runtimes than \pkg{MulticoreTSNE}. Newer implementations, such as \pkg{FIt-SNE} and \opentsne, both implement the FIt-SNE approximation scheme, making them suitable for the visualization of millions of data points. While both libraries implement the same algorithm, \opentsne\ emphasizes ease of use and extensibility and is primarily written in \proglang{Python}, which inevitably introduces some runtime overhead. This makes \opentsne\ marginally slower than its pure \proglang{C++} counterpart -- \pkg{FIt-SNE}. However, these differences disappear almost entirely when utilizing multi-threading, as shown in Fig.~\ref{fig:benchmarks_py}. Additionally, \opentsne\ provides a flexible API, allowing the user to split up the embedding-construction process into several parts. This allows us to cache and reuse intermediate results, enabling users to quickly experiment with different parameter settings and iterate on their final visualizations. This kind of functionality is not supported in other implementations, which require the user to start from scratch for every embedding, resulting in large amounts of redundant computation.

% Problem with other programming languages
In other programming languages, the selection of fast, open-source implementations of t-SNE is more limited. In \proglang{R}~\citep{team2013r}, the most widely used implementation of t-SNE -- \pkg{Rtsne}~\citep{krijthe2015rtsne} -- is based on the Barnes-Hut approximation, and is closely comparable to \pkg{MulticoreTSNE}. In \proglang{Julia}~\citep{bezanson2017julia}, the most popular implementation -- \pkg{TSne.jl}~\citep{julia_tsne} -- is based on a naive $\mathcal{O}(N^2)$ implementation of the algorithm, which allows it to be used only on smaller data sets containing up to thousands of data points. We benchmark \pkg{Rtsne} (v0.15) and  \pkg{TSne.jl} (v1.3.0) and compare it to \opentsne\ (v0.6.2). Fig.~\ref{fig:benchmarks_lang} illustrates the scaling of each approximation scheme. Of these implementations, only \opentsne\ is able to scale to data sets containing millions of data points in a reasonable amount of time.
...
\begin{figure*}[ht]
  ...
  algorithm from the \proglang{Python}, \proglang{R}, and \proglang{Julia}
  programming languages. We choose
  \opentsne\ (v0.6.2) for \proglang{Python}, \pkg{Rtsne}~\citep{krijthe2015rtsne} (v0.15)
  for \proglang{R}, and \pkg{TSne.jl}~\citep{julia_tsne} (v1.3.0) for
  \proglang{Julia}. The benchmarks reflect the time complexity of the
  ...
  \pkg{MulticoreTSNE} in \proglang{Python}, as it implements the $\mathcal{O}(N \log N)$
...
thousands of data records. Other \proglang{C++} implementations such as \pkg{MulticoreTSNE}
...
operating system, where the \proglang{C++} compiler does not come with the system, making
...
precompiled binaries for all major \proglang{Python} versions on all major platforms,
making the installation process as seamless as possible. One can install
\opentsne\ through the \proglang{Python} Package Index (\pkg{PyPI}) or \pkg{conda} from the
...
established in the \proglang{Python} data science ecosystem. \opentsne\ implements
...
points. While the \proglang{Python} virtual machine inevitably introduces some performance
overhead, the runtime is comparable to its \proglang{C++} counterpart -- \pkg{FIt-SNE}.
...
comparison to other popular t-SNE implementations in Tab.~\ref{tab:features}.
...
We introduce \opentsne, the most feature-complete, open-source, \proglang{Python}
