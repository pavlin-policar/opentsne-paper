...
\subsection{Structure of openTSNE}

\opentsne\ is designed to accomodate all users, including both a simple API that allows users to quickly obtain standard t-SNE visualization, as well as an advanced API that allows practitioners and researchers to modify different aspects of the t-SNE algorithm.

The t-SNE algorithm is dependent on three key steps: the initial positions of points in the embedding, an affinity model describing the similarities between data points in the ambient space, and the optimization which optimizes the embedding layout. \opentsne\ mirrors these different components in its design. \opentsne\ contains three corresponding core modules -- \code{initialization}, \code{affinity}, and \code{tsne} -- each of which will be decribed in detail in later sections. These modules are designed to be easily extensible, allowing researchers to experiment with different configurations and produce the best possible t-SNE visualizations. For less demanding users, \opentsne\ also provides a familiar, scikit-learn-like interface through the \code{openTSNE.TSNE} class.
...
\subsubsection{Embedding objects}
...
$\mathcal{O}(N)$. This approximation scheme is included in \pkg{FIt-SNE} and \opentsne, enabling the embedding of large data sets in a matter of minutes.
...
\pkg{FIt-SNE} (v1.1.0), and our \opentsne\ (v0.6.1). We perform benchmarks on
...
\begin{figure*}[ht]
  \includegraphics[width=\textwidth]{benchmarks_langs}
