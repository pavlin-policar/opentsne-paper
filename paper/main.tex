...
The culprit for slow response time stems from particular optimization procedures
and their specific implementation in popular
...
asymptotic time complexity $\mathcal{O}(N^2)$~\citep{maaten2008visualizing}, or the Barnes-Hut approximation
scheme developed by \citet{van2014accelerating} with asymptotic time complexity
$\mathcal{O}(N \log N)$. Recently, however, \citet{linderman2019fast} developed
a novel approximation scheme -- FIt-SNE -- which reduces the asymptotic time
complexity to $\mathcal{O}(N)$, overcoming the difficulties of scaling t-SNE
to large data sets.

% We benchmark Python
Fig.~\ref{fig:benchmarks_py} benchmarks four popular Python t-SNE
implementations, including \pkg{scikit-learn} (v1.0.1), \pkg{MulticoreTSNE}
(v0.1), \pkg{FIt-SNE} (v1.1.0), and \opentsne\ (v0.6.2). Benchmarks were
...
% Problem with Python
In Python, the most widely-used implementation of t-SNE comes from \pkg{scikit-learn},
which exhibits long runtimes when compared to other Python implementations.
\pkg{scikit-learn}, like its C++ counterpart -- \pkg{MulticoreTSNE}~\citep{Ulyanov2016} ,
is based on the Barnes-Hut approximation scheme. However, as
shown on Fig.~\ref{fig:benchmarks_py}, \pkg{scikit-learn} exhibits much longer
runtimes than \pkg{MulticoreTSNE}.
Newer implementations, such as \pkg{FIt-SNE} and \opentsne, both implement the
FIt-SNE approximation scheme, making them suitable for the visualization of
millions of data points. While both libraries implement the same
algorithm, \opentsne\ emphasizes ease of use and extensibility, and is primarily
written in Python, which inevitably introduces some runtime overhead. This
makes \opentsne\ marginally slower than its pure C++ counterpart -- \pkg{FIt-SNE}.
However, these differences disappear almost entirely when
utilizing multi-threading, as shown on Fig.~\ref{fig:benchmarks_py}.
Additionally, \opentsne\ provides a flexible API, allowing the user to split up
the embedding-construction process into several parts. This allows us to cache
and reuse intermediate results, enabling users to quickly experiment with
different parameter settings and iterate on their final visualizations. This
kind of functionality is not supported in other implementations, which require
the user to start from scratch for every embedding, resulting in large amounts
of redundant computation.


% Problem with other programming languages
In other programming languages, the selection of fast, open-source
implementations of t-SNE is more limited. In R, the most widely used
implementation of t-SNE -- \pkg{Rtsne}~\citep{krijthe2015rtsne} -- is based on
the Barnes-Hut approximation, and is closely comparable to 
\pkg{MulticoreTSNE}. In Julia, the most popular implementation -- 
\pkg{TSne.jl}~\citep{julia_tsne} -- is based on a naive $\mathcal{O}(N^2)$ implementation of the
algorithm, which allows it to be used only on smaller data sets containing up
to thousands of data points. We benchmark \pkg{Rtsne} (v0.15) and 
\pkg{TSne.jl} (v1.3.0) and compare it to \opentsne\ (v0.6.2).
Fig.~\ref{fig:benchmarks_lang} illustrates the scaling of each approximation 
scheme. Of these implementations, only \opentsne\ is able to scale to data sets
containing millions of data points in a reasonable amount of time.
...
\begin{figure*}[ht]
  ...
  algorithm from the Python, R, and Julia programming languages. We choose
  \opentsne\ (v0.6.2) for Python, \pkg{Rtsne}~\citep{krijthe2015rtsne} (v0.15)
  for R, and \pkg{TSne.jl}~\citep{julia_tsne} (v1.3.0) for Julia. The benchmarks
  reflect the time complexity of the
  implemented approximation schemes. For instance, \pkg{TSne.jl} implements a
  naive $\mathcal{O}(N^2)$ algorithm, making it prohibitively expensive for all
  but the smallest of data sets. \pkg{Rtsne} scales similarily to
  \pkg{MulticoreTSNE} in Python, as it implements the $\mathcal{O}(N \log N)$
  Barnes-Hut approximation scheme. Interestingly, the multi-threaded
  implementation version of \pkg{Rtsne} crashes on benchmark data sets
  containing over 500,000 data points due to lack of stack memory. We found
  these crashes to be inconsistent and somwewhat random. \opentsne\ is the only
...
\caption{\label{tab:features}
  We compare the features of \opentsne~(v0.6.2) to \pkg{scikit-learn}~(v1.0.1),
...
\pkg{openTSNE}~v0.6.2 package. All results and figures can be reproduced using
