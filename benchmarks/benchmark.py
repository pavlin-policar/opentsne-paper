...
for size in ${SAMPLE_SIZES[@]}; do
    cmd="OMP_NUM_THREADS=1 NUMBA_NUM_THREADS=1 python benchmark.py $METHOD run_multiple --n $REPETITIONS --n-samples $size 2>&1 | tee -a logs/${METHOD}_${size}.log";
    echo "$cmd";
    eval "$cmd";
done;
...
class UMAP(TSNEBenchmark):
    def run(self, n_samples=1000, random_state=None):
        ...
        start = time.time()
        umap.UMAP(random_state=random_state, verbose=True).fit_transform(x)
